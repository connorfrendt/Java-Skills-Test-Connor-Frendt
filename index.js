        ...
        .then(({ selection }) => {
            ...
            if(selection === 'Daily Sales Report') {
                ...
                if(money.totalRevenue > Math.abs(money.totalCost)) {
                    var surplus = money.totalRevenue - Math.abs(money.totalCost);
                    console.log('TOTAL PROFIT: ', surplus.toFixed(2));
                    console.log('You have made $', surplus.toFixed(2), 'in profit today');
                }

                else if(money.totalRevenue < Math.abs(money.totalCost)) {
                    var shortage = Math.abs(money.totalCost) - money.totalRevenue;
                    console.log('TOTAL PROFIT: ', shortage.toFixed(2) * -1);
                    console.log('You have costed yourself $', shortage.toFixed(2));
                }
